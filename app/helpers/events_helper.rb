module EventsHelper
  ...
  def fullpaperdeadline(event)
    ...
    if event.nofullpaper?
      ...
      html << "#{event.fullpaperdeadline.strftime("%e. %b %Y, %l:%M %p, %Z")}"
    else
      html << "#{event.fullpaperdeadline.strftime("%e. %b %Y, %l:%M %p, %Z")} (#{distance_of_time_in_words_to_now(event.fullpaperdeadline)}) left"
    end
  end
  ...
  def precisdeadline(event)
     html = ""
      if event.noprecis?
        ...
        html << "#{event.precisdeadline.strftime("%e. %b %Y, %l:%M %p, %Z")}"
      else
        html << "#{event.precisdeadline.strftime("%e. %b %Y, %l:%M %p, %Z")} (#{distance_of_time_in_words_to_now(event.precisdeadline)}) left"
      end
  end
